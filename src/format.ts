...
type SqlxFormatOptions = {
  ...
  /**
   * The warehouse to use when formatting.
   */
...
export function format(
  ...
  options: SqlxFormatOptions = {
    tabSize: 2,
    insertSpaces: true,
    warehouse: DEFAULT_WAREHOUSE_FOR_FORMATTING,
  }
) {
  try {
    switch (fileExtension) {
      case "sqlx":
        return postProcessFormattedSqlx(
          formatSqlx(SyntaxTreeNode.create(text), "", options)
        );
      case "js":
        return `${formatJavaScript(text, options).trim()}\n`;
      default:
        return text;
    }
  } catch (e) {
    throw new Error(`Unable to format "${text?.substring(0, 20)}...".`);
  }
}
...
function formatSqlx(
  ...
  const formattedSqlxStatements = sqlxStatements.map((sqlxStatement) => {
    ...
    const formattedPlaceholderSql = formatSql(
      unformattedPlaceholderSql,
      options
    );
    return formatEveryLine(
      replacePlaceholders(formattedPlaceholderSql, placeholders, options),
      (line) => `${indent}${line}`
    );
  });
...
${formatSqlx(sqlCodeBlockWithoutOuterBraces, "  ", options)}
${lastBraceOnwards}`;
  });
...
function replacePlaceholders(
  ...
  return Object.keys(placeholders).reduce(
    (partiallyFormattedSql, placeholderId) => {
      ...
      return formatPlaceholderInSqlx(
        placeholderId,
        placeholderValue,
...
function formatJavaScript(text: string, options: SqlxFormatOptions) {
...
function formatSql(text: string, options: SqlxFormatOptions) {
  const language = WAREHOUSE_LANGUAGE_MAP[options.warehouse];
...
function formatPlaceholderInSqlx(
  ...
  const formattedPlaceholder = formatSqlQueryPlaceholder(
...
function formatSqlQueryPlaceholder(
  node: SyntaxTreeNode,
  jsIndent: string,
  options: SqlxFormatOptions
): string {
  switch (node.type) {
    case SyntaxTreeNodeType.JAVASCRIPT_TEMPLATE_STRING_PLACEHOLDER:
      return formatJavaScriptPlaceholder(node, jsIndent, options);
    case SyntaxTreeNodeType.SQL_LITERAL_STRING:
    case SyntaxTreeNodeType.SQL_COMMENT:
      return formatEveryLine(
        node.concatenate(),
        (line) => `${jsIndent}${line.trimStart()}`
      );
    case SyntaxTreeNodeType.SQL_LITERAL_MULTILINE_STRING:
...
function formatJavaScriptPlaceholder(
  node: SyntaxTreeNode,
  jsIndent: string,
  options: SqlxFormatOptions
) {
  const formattedJs = formatJavaScript(node.concatenate(), options);
